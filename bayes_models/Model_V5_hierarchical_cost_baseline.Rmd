...
budget_scaledPer10k <- cpp_df$CostPerPerson[match(regions, cpp_df$Region)]

data.frame(Region = regions, CostPerPerson = round(budget_scaledPer10k, 4))
```
...
model{
  for(i in 1:I){
    ...
    alpha[i] ~ dnorm(mu_lambda * budget_scaledPer10k[i], tau_alpha)
