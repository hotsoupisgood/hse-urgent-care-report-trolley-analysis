...
class Model(BaseModel):
    ...
    def jags_model_string(self):
        ...
        model{
          for(i in 1:I){
            ...
            for(t in 1:T){
              mu[i,t] <- alpha[i] +
                         beta[i]  * cos((2 * pi) * (t/26)) +
                         gamma[i] * sin((2 * pi) * (t/26))
            }
            fullmod[i,1] <- mu[i,1]
            for(t in 2:T){
    ...
    def monitor_params(self):
        return ['alpha', 'beta', 'gamma', 'tau', 'phi', 'mu', 'fullmod', 'resid']
    ...
    def reconstruct_mu_old(self, raw_df, regions, n_weeks, indicators):
        ...
        for i in range(n_region):
            mu_i = (raw_df[f'alpha[{i + 1}]'].values[:, None]
                    + raw_df[f'beta[{i + 1}]'].values[:, None] * ev['cos_t26'][None, :]
                    + raw_df[f'gamma[{i + 1}]'].values[:, None] * ev['sin_t26'][None, :])
