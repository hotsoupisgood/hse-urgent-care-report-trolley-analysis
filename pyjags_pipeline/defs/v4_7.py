...
class Model(BaseModel):
    ...
    @property
    def name(self):
        return ('Baseline + Annual Cycle + Regional New Year (date-anchored)'
                ' + MW Reset (3 weeks + 2 post) + AR(2) + 4-Week Cycle')
          ...
          for(i in 1:I){
            ...
            for(t in 1:T){
              mu[i,t] <- alpha[i] +
                         ...
                         beta2[i]      * cos((2 * pi) * (t/4)) +
                         gamma2[i]     * sin((2 * pi) * (t/4)) +
    ...
    def monitor_params(self):
        return ['alpha', 'beta', 'gamma', 'beta2', 'gamma2', 'tau', 'phi1', 'phi2',
